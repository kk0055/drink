...
<div class="mt-1 md:mt-0 md:col-span-2 max-w-3xl mx-auto  p-16">
  ...
  <form action="{{ route('drinks.update', $item) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="shadow overflow-hidden sm:rounded-md">
      <div class="px-4 py-5 bg-white sm:p-6">
        <div class="create-text flex flex-col w-full items-center  px-6  tracking-wide">
             <h1 class="text-2xl mb-3">~編集~</h1>
          </div>
          ...
          <div class="col-span-6">
            ...
            <input type="text" name="name"  class="mt-1 p-1 focus:outline-none focus:ring  block w-full shadow-sm sm:text-sm rounded-md border border-indigo-600" placeholder="" value="{{$item->name }}" >
          </div>
          @error('name')
          ...
          <div class="col-span-6 mt-3">
            ...
            <input type="text" name="place"   class="mt-1 p-1 focus:outline-none focus:ring  block w-full shadow-sm sm:text-sm rounded-md border border-indigo-600" placeholder="" value="{{$item->place }}" >
          </div>
          @error('place')
          <div class="text-red-500 mb-2 text-sm">
            {{ $message }}
          ...
          <div class="col-span-6 mt-3 sm:col-span-6 lg:">
            ...
            <textarea type="text" name="body" id="body" class=" w-5  focus:outline-none	 w-full shadow-sm sm:text-sm border-gray-300 rounded-md border border-indigo-600"  rows="5"  placeholder=" 詳細をご記入ください" value="" >{{$item->body }}</textarea>
        ...
        @enderror 
          ...
          <div class="col-span-6 sm:col-span-6 lg:col-span-2 mt-3">
            ...
            <input type="file" name="image" id="filename" class="focus:outline-none " value="{{$item->image }}" >
          </div>
